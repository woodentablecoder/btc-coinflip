  ...
  useEffect(() => {
    if (user) {
      const initializeUser = async () => {
        ...
        while (retries > 0) {
          try {
            console.log(`Initializing user (retries left: ${retries}):`, user.id);
            ...
            if (error) {
              if (error.code === 'PGRST116') {
                ...
                console.log('User successfully created and verified:', newUser.id);
            ...
            retries--;
            if (retries === 0) {
              console.error('Failed to initialize user after multiple attempts');
            } else {
      ...
      const balanceSubscription = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'users', filter: `id=eq.${user.id}` }, fetchUserBalance)
        .subscribe();
